...
void CGUIDialogAudioDSPSettings::FrameMove()
{
  if (g_application.m_pPlayer->HasPlayer())
  {
    ...
    int  streamId = CServiceBroker::GetActiveAE().GetAudioDSP().GetActiveStreamId();
    if (m_ActiveStreamId != streamId)
    {
      m_ActiveStreamId      = streamId;
      m_ActiveStreamProcess = CServiceBroker::GetActiveAE().GetAudioDSP().GetDSPProcess(m_ActiveStreamId);
...
void CGUIDialogAudioDSPSettings::InitializeSettings()
{
  ...
  m_ActiveStreamId      = CServiceBroker::GetActiveAE().GetAudioDSP().GetActiveStreamId();
  m_ActiveStreamProcess = CServiceBroker::GetActiveAE().GetAudioDSP().GetDSPProcess(m_ActiveStreamId);
  if (m_ActiveStreamId < 0 || !m_ActiveStreamProcess)
  {
  ...
  if (modesAvailable > 0)
  {
    ...
    if (m_MasterModes[AE_DSP_ASTREAM_BASIC].size() > 1)
      modeEntries.push_back(std::pair<int, int>(CADSPUtil::GetStreamTypeName(AE_DSP_ASTREAM_BASIC),   AE_DSP_ASTREAM_BASIC));
    if (m_MasterModes[AE_DSP_ASTREAM_MUSIC].size() > 1)
      modeEntries.push_back(std::pair<int, int>(CADSPUtil::GetStreamTypeName(AE_DSP_ASTREAM_MUSIC),   AE_DSP_ASTREAM_MUSIC));
    if (m_MasterModes[AE_DSP_ASTREAM_MOVIE].size() > 1)
      modeEntries.push_back(std::pair<int, int>(CADSPUtil::GetStreamTypeName(AE_DSP_ASTREAM_MOVIE),   AE_DSP_ASTREAM_MOVIE));
    if (m_MasterModes[AE_DSP_ASTREAM_GAME].size() > 1)
      modeEntries.push_back(std::pair<int, int>(CADSPUtil::GetStreamTypeName(AE_DSP_ASTREAM_GAME),    AE_DSP_ASTREAM_GAME));
    if (m_MasterModes[AE_DSP_ASTREAM_APP].size() > 1)
      modeEntries.push_back(std::pair<int, int>(CADSPUtil::GetStreamTypeName(AE_DSP_ASTREAM_APP),     AE_DSP_ASTREAM_APP));
    if (m_MasterModes[AE_DSP_ASTREAM_MESSAGE].size() > 1)
      modeEntries.push_back(std::pair<int, int>(CADSPUtil::GetStreamTypeName(AE_DSP_ASTREAM_MESSAGE), AE_DSP_ASTREAM_MESSAGE));
    if (m_MasterModes[AE_DSP_ASTREAM_PHONE].size() > 1)
      modeEntries.push_back(std::pair<int, int>(CADSPUtil::GetStreamTypeName(AE_DSP_ASTREAM_PHONE),   AE_DSP_ASTREAM_PHONE));
    if (modesAvailable > 1 && m_MasterModes[m_streamTypeUsed].size() > 1)
      modeEntries.insert(modeEntries.begin(), std::pair<int, int>(CADSPUtil::GetStreamTypeName(AE_DSP_ASTREAM_AUTO), AE_DSP_ASTREAM_AUTO));
  }
  ...
  for (unsigned int i = 0; i < m_MasterModes[m_streamTypeUsed].size(); i++)
  {
    if (m_MasterModes[m_streamTypeUsed][i])
    {
      ...
      else if (CServiceBroker::GetActiveAE().GetAudioDSP().GetAudioDSPAddon(m_MasterModes[m_streamTypeUsed][i]->AddonID(), addon))
      {
   ...
   */
  {
    ...
    for (unsigned int i = 0; i < m_MasterModes[m_streamTypeUsed].size(); i++)
    {
      ...
      {
        AE_DSP_ADDON addon;
        if (CServiceBroker::GetActiveAE().GetAudioDSP().GetAudioDSPAddon(m_MasterModes[m_streamTypeUsed][i]->AddonID(), addon))
        {
          AE_DSP_MENUHOOKS hooks;
          if (CServiceBroker::GetActiveAE().GetAudioDSP().GetAudioDSPMenuHooks(m_MasterModes[m_streamTypeUsed][i]->AddonID(), AE_DSP_MENUHOOK_MASTER_PROCESS, hooks))
          {
   ...
   */
  {
    ...
    {
      AE_DSP_ADDON addon;
      if (CServiceBroker::GetActiveAE().GetAudioDSP().GetAudioDSPAddon(m_ActiveModes[i]->AddonID(), addon))
      {
        std::string label;
        ...
        if (CServiceBroker::GetActiveAE().GetAudioDSP().GetAudioDSPMenuHooks(m_ActiveModes[i]->AddonID(), AE_DSP_MENUHOOK_INFORMATION, hooks))
        {
          for (unsigned int j = 0; j < hooks.size(); j++)
          {
...
bool CGUIDialogAudioDSPSettings::HaveActiveMenuHooks(AE_DSP_MENUHOOK_CAT category)
{
  ...
  if (CServiceBroker::GetActiveAE().GetAudioDSP().HasAudioDSPMenuHooks(category) &&
      CServiceBroker::GetActiveAE().GetAudioDSP().GetEnabledAudioDSPAddons(addonMap) > 0)
  {
    for (AE_DSP_ADDONMAP_ITR itr = addonMap.begin(); itr != addonMap.end(); itr++)
    {
      AE_DSP_MENUHOOKS hooks;
      if (CServiceBroker::GetActiveAE().GetAudioDSP().GetAudioDSPMenuHooks(itr->second->GetID(), category, hooks))
      {
...
std::string CGUIDialogAudioDSPSettings::GetSettingsLabel(CSetting *pSetting)
{
  if (pSetting->GetLabel() == 15041)
  {
    ...
    if (settingId.substr(0, 27) == SETTING_STREAM_INFO_MODE_CPU_USAGE)
    {
      ...
      if (ptr >= 0 && CServiceBroker::GetActiveAE().GetAudioDSP().GetAudioDSPAddon(m_ActiveModes[ptr]->AddonID(), addon))
    ...
    else
    {
      ...
      if (ptr >= 0 && CServiceBroker::GetActiveAE().GetAudioDSP().GetAudioDSPAddon(m_Menus[ptr].addonId, addon))
...
{
  AE_DSP_ADDONMAP addonMap;
  if (CServiceBroker::GetActiveAE().GetAudioDSP().GetEnabledAudioDSPAddons(addonMap) > 0)
  {
    for (AE_DSP_ADDONMAP_ITR itr = addonMap.begin(); itr != addonMap.end(); itr++)
    {
      AE_DSP_MENUHOOKS hooks;
  ...
  {
    AE_DSP_ADDON addon;
    if (CServiceBroker::GetActiveAE().GetAudioDSP().GetAudioDSPAddon(m_Menus[i].addonId, addon) && category == m_Menus[i].hook.category)
    {
...
bool CGUIDialogAudioDSPSettings::OpenAudioDSPMenu(unsigned int setupEntry)
{
  ...
  AE_DSP_ADDON addon;
  if (!CServiceBroker::GetActiveAE().GetAudioDSP().GetAudioDSPAddon(m_Menus[setupEntry].addonId, addon))
    return false;
